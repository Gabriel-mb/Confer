...
public class EpiInputsController {
    ...
    public void onIncludeButtonClick() throws SQLException, IOException {
        ...
        if (epiName.getValue() == null) {
            ...
            Epi epi = epiDAO.searchEpi(caSeparated);
            ...
            ObservableList<Epi> episBorrowed = epiDAO.episListBorrowed(parseInt(idLabel.getText()));
            for (Epi item : episBorrowed) {
                if (item.getNumCa().equals(numCa) && item.getDate().equals(selectedDate)) {
                    ...
                    epiDAO.updateStock(item.getEpiName(), item.getNumCa(), updatedStock);
                    int updatedQuantity = parseInt(epiQuantity.getText()) + item.getQuantity();
                    epiDAO.updateBorrowedEpi(updatedQuantity, item.getEpiName(), item.getNumCa(), Date.valueOf(date.getValue()), Integer.valueOf(idLabel.getText()));
                    terminate = true;
                }
            }
            if (!terminate) {
                int updatedStock = epi.getQuantity() - parseInt(epiQuantity.getText());
                epiDAO.updateStock(epi.getEpiName(), epi.getNumCa(), updatedStock);
                epiDAO.createBorrowed(new Epi(parseInt(epiQuantity.getText()), epi.getNumCa(), epi.getEpiName(), Date.valueOf(date.getValue())), parseInt(idLabel.getText()));
            ...
            table.setItems(epiDAO.episListBorrowed(parseInt(idLabel.getText())));
        }
    }
    ...
    public void onRemoveButtonClick() throws SQLException, IOException {
        ...
        if (selectedBorrowedList != null && !selectedBorrowedList.isEmpty()) {
            for (Epi selectedBorrowed : selectedBorrowedList) {
                episList = epiDAO.episListBorrowed(parseInt(idLabel.getText()));
                if (epiDAO.checkDate(selectedBorrowed)) {
                    ...
                    if (result.isPresent() && result.get() == yesButton) {
                        epiDAO.remove(selectedBorrowed.getEpiName(), selectedBorrowed.getNumCa(), selectedBorrowed.getDate());
    ...
    public void setTable() throws SQLException, IOException {
        Connection connection = new ConnectionDAO().connect();
        EpiDAO epiDAO = new EpiDAO(connection);
        ObservableList<Epi> epiList = FXCollections.observableList(epiDAO.episListBorrowed(Integer.valueOf(idLabel.getText())));
        ...
        date.setRowCellFactory(Epi -> new MFXTableRowCell<>(models.Epi::getDate));
